...
The two flow accents are **reserved for token motion** (PRD §6.1) — the travelling token chips and flow arrows, never buttons, links, or chrome. Cool = context tokens travelling **right into the LLM**; warm = the generated response streaming **left back to the agent**. That consistency is the navigational cue, so don't dilute it.
...
- **Standalone chapter sections, no camera path.** Each chapter is its own full in-page section with a bespoke visual. Within a section, sticky/pinned stages stepped by ScrollTrigger are fine — discrete states on step entry, never continuous animation bound to scroll position.
- **Detail fades in on entry.** Labels and sub-mechanisms appear only when their section or step activates (staggered reveals via timeline delays); they fade back to `--faint`/hidden when it leaves. Keep any single view uncluttered.
...
- **Conceptual chapters break away.** Tokenization, embeddings, and hallucinations get full-bleed bespoke canvases away from the machine schematic.
...
- Static-first build (adapter-static): each chapter maps to a real in-page anchor so the narrative is navigable without JS.
