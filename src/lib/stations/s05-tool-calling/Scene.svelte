<script lang="ts">
	...
	const meta = manifest.find((entry) => entry.meta.id === 'tool-calling')!.meta;
...
<section bind:this={root} class="scene">
	...
	<div class="diagram-shell" data-diagram-panel>
		<DiagramPanel label="Stub diagram" caption="A placeholder for the inner loop">
			<p>
				A tool call is still a model-generated request, but the API exposes it as a structured item with
				a tool name and arguments. The harness checks that request, runs the tool, and adds the result to
				the next model call. This stub will later show the inner loop before its full diagram arrives.
			</p>
		</DiagramPanel>
	</div>
...
<style>
	...
	.port {
		...
		height: 5rem;
		place-items: center;
		border: 1px solid var(--concept-tools);
		border-radius: 8px;
