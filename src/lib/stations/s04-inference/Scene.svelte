...
<div class="scene" bind:this={root}>
	...
	<DiagramPanel label="Station outline" caption="A model makes one choice, appends it, and reads the whole packet again.">
		<p class="placeholder" data-diagram-panel>
			<strong>The answer is assembled one guess at a time.</strong> This station treats the model as a calm instrument: it scores possible next tokens, selects one candidate, and appends it before reading the growing sequence again. Repeating that cycle turns a stream of small predictions into a response that can even request a tool.
		</p>
	</DiagramPanel>
	<div class="ports" aria-label="Station ports">
		<div class="port" data-port="context-in" aria-hidden="true"><span>context in</span></div>
