<script lang="ts">
	...
	const station = manifest.find((entry) => entry.meta.id === 'agent-loop');
	...
	onMount(() => {
		if (!station) throw new Error('Missing manifest entry for agent-loop');
		register({
			meta: station.meta,
			sceneEl: root,
			ports: {
...
<div class="scene" bind:this={root}>
	<StationHead number={1} title="The agent loop" accent="user" />
	<DiagramPanel label="Station outline" caption="A request starts the loop; the handoff matters as much as the reply.">
		<p class="placeholder" data-diagram-panel>
			<strong>One request, many moving parts.</strong> This station introduces the agent loop: a request enters, the model responds, and the harness decides what happens next. We will follow one full turn so the chat feels like one moving part of a larger machine.
		</p>
	</DiagramPanel>
	<div class="ports" aria-label="Station ports">
		<div class="port" data-port="prompt-out" aria-hidden="true"><span>prompt out</span></div>
		<div class="port" data-port="answer-in" aria-hidden="true"><span>answer in</span></div>
	</div>
</div>
