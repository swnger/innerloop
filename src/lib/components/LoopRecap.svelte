<script lang="ts">
    ...
    /* ============================================================
       Lap close — the recap. The fully-detailed loop diagram,
       relocated from the opener to the end of the loop-spine: after
       visiting every station, the whole cycle reassembles here and
       resumes its auto-loop as the synthesizing payoff.
    ============================================================ */
        ...
        Promise.all([import("gsap"), import("gsap/MotionPathPlugin")]).then(
            ([core, motion]) => {
                ...
                context = gsap.context(() => {
                    ...
                    // Reduced motion: the static end-state still teaches —
                    // every band filled, all steps lit, the flow paths as a
                    // readable map. No auto-loop.
                    if (
                        window.matchMedia("(prefers-reduced-motion: reduce)")
                            .matches
                    ) {
                        gsap.set(q(".flow-chip, .gen-loop-chip"), {
                            opacity: 0,
                        });
                        gsap.set(q(".transformer-step, .generated-token"), {
                            opacity: 1,
                        });
                        phase = 7;
                        used = "4.2k";
...
<section id="recap" class="hero" aria-labelledby="recap-title">
    <div class="intro chapter-head">
        <p class="eyebrow">Lap complete · the recap</p>
        <h2 id="recap-title">That’s all it is.</h2>
        <p class="recap-lede">
            The window you watched fill, the tokens, the guessing, the tool
            runs — here they are again, running together as one turn.
        </p>
    </div>
    ...
    <figure class="stage" bind:this={stageEl}>
        ...
        >
            ...
            <g>
                ...
                <path d="M 841 380 L 841 466" fill="none" stroke={FAINT} stroke-width="1" stroke-dasharray="2 3" marker-end="url(#rc-arrow-faint)" />
