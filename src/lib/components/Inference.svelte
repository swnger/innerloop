<script lang="ts">
	...
	const REPLY = ['The', 'test', 'passes', 'now', '—', 'the', 'bug', 'was', 'a', 'missing', 'null', 'check', '.'];
	...
	type NTPreset = { label: string; prompt: string[]; table: Record<string, Cand[]> };
	const NT_PRESETS: NTPreset[] = [
		{ label: 'the classic', prompt: ['The', 'cat', 'sat', 'on', 'the'], table: CAT_TABLE },
		{ label: 'a story opener', prompt: ['Once', 'upon', 'a'], table: STORY_TABLE },
		{ label: 'a “fact”', prompt: ['The', 'capital', 'of', 'France', 'is'], table: FRANCE_TABLE },
		{ label: 'half a quote', prompt: ['To', 'be', 'or', 'not', 'to'], table: BARD_TABLE }
	];
	const MAX_GEN = 26;
	...
	const restMass = $derived(Math.max(0, 1 - cands.reduce((s, c) => s + c.p, 0)));
	const sameNote = $derived(
		greedy &&
			runs.length >= 2 &&
			runs[runs.length - 1].tokens.join(' ') === runs[runs.length - 2].tokens.join(' ')
	);
	...
	function candidatesFor(tokens: string[], table: Record<string, Cand[]>): Cand[] {
	...
	const FK_BRANCHES: FkBranch[] = [
		...
		{ p: '.09', toks: ['keyboard', 'and', 'refused', 'to', 'move', '.'], y: 306 }
	];
		...
		Promise.all([
			...
			context = gsap.context(() => {
				...
				gsap
					...
					.from('.open-claim-2', { autoAlpha: 0, y: 20, duration: 0.4 }, '<0.3')
					.addLabel('chips')
					.to({}, { duration: 0.4 })
				...
				gsap.fromTo(
					...
					{ opacity: 0.12 },
					{
						...
						stagger: 0.05,
						ease: 'none',
						scrollTrigger: { trigger: '.nt-head', start: 'top 78%', end: 'top 28%', scrub: true }
					}
				);

				/* ---- section seams ---- */
				gsap.utils.toArray('.nt-seam', rootEl).forEach((seam: Element) => {
					gsap
				...
				an
					...
					.from('.an-chip', { autoAlpha: 0, y: 14, duration: 0.4, stagger: 0.07 }, '<0.1')
					.from('.an-slot1', { autoAlpha: 0, duration: 0.3 }, '<0.3')
					.from('.cap-read', { autoAlpha: 0, y: 10, duration: 0.35 }, '<')
					.addLabel('read')
					.to({}, { duration: 0.5 })
					...
					.from('.d1 .bar-meta', { autoAlpha: 0, duration: 0.3, stagger: 0.06 }, '<0.15')
					.from('.d1 .bar-tail', { autoAlpha: 0, duration: 0.3 })
					.from('.cap-score', { autoAlpha: 0, y: 10, duration: 0.35 }, '<')
					.addLabel('score')
					.to({}, { duration: 0.5 })
					...
					.to('.pick-marker', { y: BAR_DY * 4, duration: 0.6, ease: 'power1.inOut' })
					.to('.pick-marker', { y: BAR_DY * 1, duration: 0.45, ease: 'back.out(1.2)' })
					...
					.to('.nt-fly1', {
						duration: 0.8,
						ease: 'power1.inOut',
						motionPath: { path: '#nt-fly1-path', align: '#nt-fly1-path', alignOrigin: [0.5, 0.5] }
					})
					.set('.nt-fly1', { autoAlpha: 0 })
					.to('.an-slot1-dash', { autoAlpha: 0, duration: 0.2 }, '<0.6')
					.fromTo(
						'.an-slot1-fill',
					...
					.from('.cap-append', { autoAlpha: 0, y: 10, duration: 0.35 })
					.addLabel('append')
					.to({}, { duration: 0.5 })
					...
					.to('.an-slot1-fill text', { fill: '#F1F4F7', duration: 0.5 }, '<')
					...
					.from('.d2 .bar-meta', { autoAlpha: 0, duration: 0.25, stagger: 0.05 }, '<0.1')
					...
					.to('.nt-fly2', {
						duration: 0.6,
						ease: 'power1.inOut',
						motionPath: { path: '#nt-fly2-path', align: '#nt-fly2-path', alignOrigin: [0.5, 0.5] }
					})
					.set('.nt-fly2', { autoAlpha: 0 })
					.to('.an-slot2-dash', { autoAlpha: 0, duration: 0.15 }, '<0.45')
					.fromTo(
						'.an-slot2-fill',
						{ autoAlpha: 0, scale: 0.7, transformOrigin: '50% 50%' },
					...
					.to('.an-slot2-fill rect', { fill: '#1A2634', stroke: '#4B5C6E', duration: 0.35 })
					.to('.an-slot2-fill text', { fill: '#F1F4F7', duration: 0.35 }, '<')
					...
					.from('.d3 .bar-meta', { autoAlpha: 0, duration: 0.25, stagger: 0.05 }, '<0.1')
					...
					.from('.an-stop', { autoAlpha: 0, scale: 0.8, transformOrigin: '50% 50%', duration: 0.35 })
					.to('.an-rowglow', { autoAlpha: 0.85, duration: 0.4 }, '<')
					.from('.cap-stop', { autoAlpha: 0, y: 10, duration: 0.35 }, '<')
					.addLabel('stop')
					.to({}, { duration: 0.7 });

				/* ---- 4 · lab reveal ---- */
				gsap
					.timeline({ scrollTrigger: { trigger: '.nt-lab-sec', start: 'top 72%' } })
					.from('.nt-lab-reveal', {
						opacity: 0,
						y: 30,
						duration: 0.6,
					...
					.from('.guess-lab', { opacity: 0, y: 50, duration: 0.7, ease: 'power2.out' }, '<0.2');

				/* ---- 5 · the fork: one draw rewrites the future ---- */
				const fkCurves = gsap.utils.toArray('.fk-curve', rootEl) as SVGPathElement[];
				fkCurves.forEach((p) => {
				...
				fk.from('.fk-trunk .fk-chip', { autoAlpha: 0, y: 12, duration: 0.35, stagger: 0.06 });
				FK_BRANCHES.forEach((_, bi) => {
					fk.to(fkCurves[bi], { strokeDashoffset: 0, duration: 0.55, ease: 'power1.inOut' }, bi ? '>-0.15' : '>')
						.from(`.fk-p-${bi}`, { autoAlpha: 0, duration: 0.25 }, '<0.3')
						.from(`.fk-b-${bi} .fk-chip`, { autoAlpha: 0, y: 10, duration: 0.3, stagger: 0.07 }, '<0.1');
				});
...
<section id="prediction" class="nt" data-chapter="03" bind:this={rootEl} aria-labelledby="nt-title">
	{#snippet seam()}
	...
	<div class="nt-open">
		<div class="nt-open-stage">
			<p class="open-kicker mono">back inside the machine · the reply you watched stream in</p>

			<div class="open-panel">
				<p class="open-panel-head mono">AGENT → YOU · one turn, closing</p>
				<p class="open-reply" aria-label="The test passes now — the bug was a missing null check.">
					{#each REPLY as t, i (i)}<span class="open-tok mono">{t}</span>{/each}<span
						class="open-caret"><span class="open-caret-inner"></span></span>
				</p>
			</div>
			...
			<div class="open-below">
				...
				<p class="open-claim open-claim-2">
					It’s guessing — one token at a time, each guess feeding the next.
				</p>
	...
	<div class="nt-head">
		<p class="nt-lede">
			{#each LEDE as w, i (i)}<span
					class="lede-w"
					class:lede-strong={['guess', 'inference'].includes(w.replace(/[^a-z]/gi, ''))}>{w}</span
				>{' '}{/each}
		</p>
	</div>
	...
	<div class="nt-anatomy">
		<div class="an-stage">
			<p class="an-kicker mono">one guess, slowed down · the only move it has</p>
			<svg
				class="an-svg"
				viewBox="0 0 920 580"
				role="img"
			...
			>
				<defs>
					<marker id="nt-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto">
				...
				{#each ROW_TOKENS as tk (tk.x)}
					<g class="an-chip">
						<rect x={tk.x} y={ROW_Y} width={tk.w} height={ROW_H} rx="6" fill="var(--token-fill)" stroke="var(--line-bright)" />
						<text x={tk.x + tk.w / 2} y={ROW_Y + ROW_H / 2 + 1} text-anchor="middle" dominant-baseline="middle" font-family="var(--mono)" font-size="13" fill="var(--paper)">{tk.t}</text>
					</g>
				{/each}
				...
				<g class="an-slot1">
					<g class="an-slot1-dash">
						<rect x={SLOT1_X} y={ROW_Y} width={SLOT1_W} height={ROW_H} rx="6" fill="none" stroke="var(--faint)" stroke-dasharray="4 4" />
						<text x={SLOT1_X + SLOT1_W / 2} y={ROW_Y + ROW_H / 2 + 1} text-anchor="middle" dominant-baseline="middle" font-family="var(--mono)" font-size="13" fill="var(--faint)">?</text>
					</g>
				</g>
				<g class="an-slot1-fill" style="opacity:0">
					<rect x={SLOT1_X} y={ROW_Y} width={SLOT1_W} height={ROW_H} rx="6" fill="var(--warm-soft)" stroke="var(--warm)" />
					<text x={SLOT1_X + SLOT1_W / 2} y={ROW_Y + ROW_H / 2 + 1} text-anchor="middle" dominant-baseline="middle" font-family="var(--mono)" font-size="13" fill="var(--warm)">sofa</text>
				</g>

				<!-- slot 2 -->
				<g class="an-slot2">
					<g class="an-slot2-dash">
						<rect x={SLOT2_X} y={ROW_Y} width={SLOT2_W} height={ROW_H} rx="6" fill="none" stroke="var(--faint)" stroke-dasharray="4 4" />
						<text x={SLOT2_X + SLOT2_W / 2} y={ROW_Y + ROW_H / 2 + 1} text-anchor="middle" dominant-baseline="middle" font-family="var(--mono)" font-size="13" fill="var(--faint)">?</text>
					</g>
				</g>
				<g class="an-slot2-fill" style="opacity:0">
					<rect x={SLOT2_X} y={ROW_Y} width={SLOT2_W} height={ROW_H} rx="6" fill="var(--warm-soft)" stroke="var(--warm)" />
					<text x={SLOT2_X + SLOT2_W / 2} y={ROW_Y + ROW_H / 2 + 1} text-anchor="middle" dominant-baseline="middle" font-family="var(--mono)" font-size="13" fill="var(--warm)">.</text>
				</g>

				<!-- the reply ships back: warm underline on the finished row -->
				<line class="an-rowglow" x1="78" y1={ROW_Y + ROW_H + 10} x2={SLOT2_X + SLOT2_W + 2} y2={ROW_Y + ROW_H + 10} stroke="var(--warm)" stroke-width="2" opacity="0" style="filter: drop-shadow(var(--glow-warm))" />

				<!-- reply-complete stamp -->
				<g class="an-stop" transform="rotate(-3 745 56)">
					<rect x="618" y="38" width="254" height="36" rx="4" fill="rgba(255,157,77,.07)" stroke="var(--warm)" />
					<text x="745" y="57" text-anchor="middle" dominant-baseline="middle" font-family="var(--mono)" font-size="11" letter-spacing="0.12em" fill="var(--warm)">REPLY COMPLETE · SENT BACK</text>
				</g>

				<!-- the model, abstracted to one pass -->
				<g class="an-model">
					<rect class="an-model-core" x="80" y="170" width="760" height="46" rx="8" fill="var(--transformer-fill)" stroke="var(--line-bright)" stroke-width="1.5" />
					<text x="100" y="194" dominant-baseline="middle" font-family="var(--mono)" font-size="11" font-weight="600" letter-spacing="0.14em" fill="var(--paper)">THE WHOLE MODEL</text>
					<text x="820" y="194" text-anchor="end" dominant-baseline="middle" font-family="var(--mono)" font-size="9.5" fill="var(--faint)">billions of weights · one full pass per token</text>
				</g>
				<line class="an-arrow-1" x1="292" y1="134" x2="292" y2="162" stroke="var(--faint)" stroke-width="1" marker-end="url(#nt-arrow)" />
				<line class="an-arrow-2" x1="292" y1="220" x2="292" y2="252" stroke="var(--faint)" stroke-width="1" marker-end="url(#nt-arrow)" />
					...
					<g class="dist d{di + 1}" style={di > 0 ? 'opacity:0' : ''}>
						{#each dist.rows as r, i (r.t)}
							<g class="bar-row row-{i}">
								<text class="bar-meta" x={BAR_X - 12} y={rowY(i) + 11} text-anchor="end" font-family="var(--mono)" font-size="12.5" fill="var(--paper)">{r.t}</text>
								<rect class="bar-rect" x={BAR_X} y={rowY(i)} width={r.p * BAR_W} height="20" rx="3" fill="var(--bar-fill)" stroke="var(--line)" />
								<text class="bar-meta bar-val" x={BAR_X + r.p * BAR_W + 10} y={rowY(i) + 11} dominant-baseline="middle" font-family="var(--mono)" font-size="10.5" fill="var(--muted)">{r.p.toFixed(2)}</text>
							</g>
						{/each}
						<text class="bar-tail" x={BAR_X} y={rowY(dist.rows.length) + 8} font-family="var(--mono)" font-size="9.5" fill="var(--faint)">{dist.tail}</text>
					</g>
				{/each}

				<!-- draw marker -->
				<g class="pick-marker" transform="translate(246 {rowY(0) + 3})" style="opacity:0">
					<path d="M0,0 L12,7 L0,14 Z" fill="var(--brand-strong)" />
				</g>

				<!-- travelling picks -->
				<g class="nt-fly1" style="opacity:0; filter: drop-shadow(var(--glow-warm))">
					<rect x="-30" y="-13" width="60" height="26" rx="6" fill="var(--warm-soft)" stroke="var(--warm)" />
					<text y="1" text-anchor="middle" dominant-baseline="middle" font-family="var(--mono)" font-size="12" fill="var(--warm)">sofa</text>
				</g>
				<g class="nt-fly2" style="opacity:0; filter: drop-shadow(var(--glow-warm))">
					<rect x="-16" y="-13" width="32" height="26" rx="6" fill="var(--warm-soft)" stroke="var(--warm)" />
					<text y="1" text-anchor="middle" dominant-baseline="middle" font-family="var(--mono)" font-size="12" fill="var(--warm)">.</text>
				</g>

				<!-- beat captions -->
				<text class="an-cap cap-read" x="460" y="548" text-anchor="middle" font-family="var(--mono)" font-size="12.5" fill="var(--muted)">1 · it reads everything so far — just tokens, nothing else</text>
				<text class="an-cap cap-score" x="460" y="548" text-anchor="middle" font-family="var(--mono)" font-size="12.5" fill="var(--muted)" style="opacity:0">2 · one pass later: a score for every token it knows</text>
				<text class="an-cap cap-pick" x="460" y="548" text-anchor="middle" font-family="var(--mono)" font-size="12.5" fill="var(--muted)" style="opacity:0">3 · a weighted draw — the favorite usually wins. not this time: sofa</text>
				<text class="an-cap cap-append" x="460" y="548" text-anchor="middle" font-family="var(--mono)" font-size="12.5" fill="var(--muted)" style="opacity:0">4 · glued on. nothing is saved anywhere — the sentence itself is the memory</text>
				<text class="an-cap cap-again" x="460" y="548" text-anchor="middle" font-family="var(--mono)" font-size="12.5" fill="var(--muted)" style="opacity:0">5 · yesterday’s guess is just another token now — same machine, new scores, new draw</text>
				<text class="an-cap cap-stop" x="460" y="548" text-anchor="middle" font-family="var(--mono)" font-size="12.5" fill="var(--muted)" style="opacity:0">6 · a stop token wins the draw — the reply is complete</text>
			</svg>
			<p class="disclaimer an-disclaimer">
				Illustrative numbers — a real model scores its full vocabulary, ~50,000+ tokens, on every pass.
			</p>
		</div>
	</div>
	...
	<div class="nt-lab-sec">
		...
		<p class="nt-prose nt-lab-reveal">
			Pick a prompt and let the text write itself. Run it twice — the dice rarely roll the
			same way.
		</p>
		...
		<div class="guess-lab">
			...
			<div class="gl-presets">
				{#each NT_PRESETS as p, i (p.label)}
					<button class="mono gl-preset" class:active={i === presetIdx} onclick={() => selectPreset(i)}>{p.label}</button>
				{/each}
			</div>
					...
					{#if cands.length}
						{#each cands as c, i (c.t)}
							<div class="gl-row" class:chosen={i === chosenIdx}>
								<span class="gl-tok mono">{c.t === END ? '⌁ end' : c.t}</span>
								<span class="gl-track"><span class="gl-fill" style:width="{c.p * 100}%"></span></span>
								<span class="gl-val mono">{Math.round(c.p * 100)}%</span>
							</div>
						{/each}
						{#if restMass > 0.005}
							<div class="gl-row gl-rest">
								<span class="gl-tok mono">everything else</span>
								<span class="gl-track"><span class="gl-fill" style:width="{restMass * 100}%"></span></span>
								<span class="gl-val mono">{Math.round(restMass * 100)}%</span>
							</div>
						{/if}
					{:else}
						<p class="gl-empty mono">press ▶ to ask for the first guess</p>
					{/if}
				</div>
			</div>
			...
			<div class="gl-controls">
				<div class="gl-buttons">
					<button class="mono gl-btn gl-run" onclick={run}>
						{running ? '■ stop' : finished ? '↻ run again' : '▶ write'}
					</button>
					<button class="mono gl-btn" onclick={stepOnce} disabled={running}>step</button>
					<button class="mono gl-btn" onclick={reset}>reset</button>
				</div>
				<div class="gl-mode" role="group" aria-label="How the next token is picked">
					<span class="mono gl-mode-label">pick:</span>
					<button class="mono gl-btn gl-toggle" class:active={!greedy} onclick={() => (greedy = false)}>weighted draw</button>
					<button class="mono gl-btn gl-toggle" class:active={greedy} onclick={() => (greedy = true)}>always the favorite</button>
				</div>
			</div>

			{#if runs.length}
				<div class="gl-runs">
					<span class="mono gl-runs-title">FINISHED RUNS · same prompt, separate draws</span>
					{#each runs as r, ri (ri)}
						<p class="gl-run mono">
							<span class="gl-run-n">{ri + 1}</span>
							<span class="gl-run-prompt">{preset.prompt.join(' ')}</span>{#each r.tokens as t, i (i)}<span
									class="gl-run-tok"
					...
					{/each}
					{#if sameNote}
						<p class="gl-note mono">always the favorite → the same sentence, every time. the variety lives in the dice.</p>
					{/if}
				</div>
			{/if}

			<p class="disclaimer gl-disclaimer">
				Illustrative — the scores here come from a small hand-written table, not learned
				weights. The loop is the real thing: score, draw, append, repeat until a stop token.
			</p>
		</div>
	</div>
	...
	<div class="nt-fork">
		...
		<p class="nt-prose">
			Each pick changes every score that follows. Run the same prompt twice and the replies
			part ways at a single token — there was never one “right” sentence waiting inside.
		</p>
		...
		>
			{#each FK_BRANCHES as b, bi (bi)}
				<path class="fk-curve" d={fkCurve(b)} fill="none" stroke={b.stub ? 'var(--line-bright)' : 'var(--warm)'} stroke-width="1.3" opacity={b.stub ? 0.5 : 0.55} />
				<text class="fk-p fk-p-{bi}" x="398" y={b.y - 4} text-anchor="middle" font-family="var(--mono)" font-size="10" fill="var(--faint)">{b.p}</text>
			{/each}

			<g class="fk-trunk">
				{#each FK_TRUNK as tk (tk.x)}
					<g class="fk-chip">
						<rect x={tk.x} y="44" width={tk.w} height="30" rx="6" fill="var(--token-fill)" stroke="var(--line-bright)" />
						<text x={tk.x + tk.w / 2} y="60" text-anchor="middle" dominant-baseline="middle" font-family="var(--mono)" font-size="12" fill="var(--paper)">{tk.t}</text>
					</g>
				{/each}
			</g>
					...
					{#each fkChips(b) as tk, ti (tk.x)}
						<g class="fk-chip">
							<rect
								...
								height="30"
								rx="6"
								fill={ti === 0 && !b.stub ? 'var(--warm-soft)' : 'var(--token-fill)'}
								stroke={ti === 0 && !b.stub ? 'var(--warm)' : 'var(--line-bright)'}
							/>
							<text x={tk.x + tk.w / 2} y={b.y + 16} text-anchor="middle" dominant-baseline="middle" font-family="var(--mono)" font-size="12" fill={ti === 0 && !b.stub ? 'var(--warm)' : 'var(--paper)'}>{tk.t}</text>
						</g>
					{/each}
					{#if b.stub}
						<text x={430 + fkChips(b).reduce((s, c) => s + c.w + 8, 0) + 6} y={b.y + 16} dominant-baseline="middle" font-family="var(--mono)" font-size="10.5" fill="var(--faint)">…and every future you never saw</text>
					{/if}
				</g>
			{/each}

			<text class="fk-cap" x="460" y="376" text-anchor="middle" font-family="var(--mono)" font-size="11.5" fill="var(--muted)">same prompt, same model — one draw lands differently, and every token after lives in a different sentence</text>
		</svg>
		<p class="nt-prose nt-fork-note">
			This is all “regenerate” does: roll the dice again.
		</p>
	</div>
	...
	<div class="nt-cards">
		...
		<div class="nt-card-grid">
			<article class="nt-card">
				...
				<p>
					There is no warehouse of sentences inside. Every reply is assembled one draw at
					a time — most have never been written before, by anyone.
				</p>
			</article>
			...
			<article class="nt-card">
				...
				<div class="freq" aria-hidden="true">
					...
					{#each FREQ_BARS as b (b.t)}
						<div class="freq-row">
							<span class="mono freq-tok" class:top={b.p > 50}>{b.t}</span>
							<span class="freq-track"><span class="freq-fill" class:top={b.p > 50} style:width="{b.p}%"></span></span>
							<span class="mono freq-val">{b.p}%</span>
						</div>
					{/each}
				</div>
				<p>
					It says Paris because those tokens followed that pattern, over and over, in its
					training text. Right answer — statistical reason. No lookup happened.
				</p>
			</article>
			...
			<article class="nt-card">
				...
				<div class="fluent" aria-hidden="true">
					<p class="fluent-quote">“…as shown in a landmark 2019 study.”</p>
					<span class="nt-stamp mono">fluent — but there is no study</span>
				</div>
				<p>
					The score measures how well a token <em>fits</em>, not whether it’s true.
					Fluent, confident, and wrong can land in the same sentence.
				</p>
			</article>
		</div>
	</div>
	...
	<div class="nt-outro">
		...
		<ul class="nt-facts">
			<li><span class="mono">STREAMING</span> Replies arrive word by word because they’re made word by word — you’re watching the loop run.</li>
			<li><span class="mono">REGENERATE</span> Same prompt, different answer: that’s the weighted draw, not a mood.</li>
			<li><span class="mono">SPEED &amp; COST</span> Every token is one full pass through the model — long replies take longer and cost more.</li>
		</ul>

		<details class="deeper">
			<summary><span class="mono">Go deeper</span> — where the scores come from</summary>
			<p>
				Training. The model played the same game trillions of times over mountains of
				text: hide the next token, guess it, nudge billions of internal dials toward the
				right answer. The dials are all that ships — no copy of the text, no table of
				facts, just whatever patterns it needed to keep winning the game.
			</p>
		</details>

		<details class="deeper">
			<summary><span class="mono">Go deeper</span> — logits, softmax, and the dice</summary>
			<p>
				The raw outputs are <em>logits</em> — one unbounded score per vocabulary token. A
		...
		<p class="disclaimer">
			All probabilities on this page are invented for clarity; real models compute them
			from learned weights, over full vocabularies, and they shift with every token of
			context.
		</p>
	</div>
</section>
